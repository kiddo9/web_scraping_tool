...
script, title, link, video, source, iframe, nav, footer, header, article, section, footer, main, aside, form`;
        ...
        $(selectors).each((index, element) => {
            ...
            if ($el.is("input, textarea")) {
                textContent = ((_a = $el.val()) === null || _a === void 0 ? void 0 : _a.trim()) || "";
            }
            else {
                textContent = $el.text().trim();
            }
            const attributes = {}; // Initialize attributes object
            ...
            const emailMatch = textContent.match(/([a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9._-]+)/gi); // Regex to match email addresses
            // Regex to match phone numbers (basic pattern)
            const phoneMatch = textContent.match(/(\+?\d[\d\s\-]{7,}\d)/g);
            if (emailMatch)
                emailMatch.forEach((e) => companyInfo.emails.add(e)); // Add emails to the set
            // Add phone numbers to the set
            // Note: This regex is basic and may not cover all phone formats
            if (phoneMatch)
                phoneMatch.forEach((p) => companyInfo.phones.add(p));
            // Social Media Links
            ...
            if (tag === "meta" &&
                ...
                const metaContent = ((_b = attributes.content) === null || _b === void 0 ? void 0 : _b.trim().toLowerCase()) || "";
                ...
                $('script[type="application/ld+json"]').each((_, element) => {
                    ...
                    try {
                        ...
                        if (((_a = json === null || json === void 0 ? void 0 : json.address) === null || _a === void 0 ? void 0 : _a.streetAddress) && !companyInfo.location) {
                            companyInfo.location = `${json.address.streetAddress}, ${json.address.addressLocality || ""}, ${json.address.addressCountry || ""}`;
                        }
                    }
                    catch (err) {
                        // ignore
